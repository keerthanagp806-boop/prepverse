...
class Store {
    ...
    init() {
        const seed = (0, seedData_1.getSeedData)();
    ...
    createUser(user) {
        const newUser = { ...user, id: `usr-${(0, uuid_1.v4)()}`, createdAt: new Date().toISOString() };
    ...
    getCourses(filter) {
        return this.courses.filter(c => {
            ...
            if (filter?.status && filter.status !== 'all' && c.status !== filter.status)
                return false;
            if (filter?.category && filter.category !== 'all' && c.category !== filter.category)
                return false;
            if (filter?.difficulty && filter.difficulty !== 'all' && c.difficulty !== filter.difficulty)
                return false;
            if (filter?.search) {
    ...
    updateLessonProgress(userId, courseId, lessonId, markComplete) {
    ...
    getAssessments(filter) {
        return this.assessments.filter(a => {
            if (filter?.category && a.category !== filter.category)
                return false;
            if (filter?.difficulty && a.difficulty !== filter.difficulty)
                return false;
            if (filter?.search) {
                const q = filter.search.toLowerCase();
    ...
    getAssessmentById(id) { return this.assessments.find(a => a.id === id); }
    createAssessment(asm) {
        const newAsm = {
            ...asm,
            id: `asm-${(0, uuid_1.v4)().slice(0, 8)}`,
            createdAt: new Date().toISOString()
        };
    ...
    getAttemptById(id) { return this.attempts.find(a => a.id === id); }
    getUserAttempts(userId) { return this.attempts.filter(a => a.userId === userId); }
    startAssessmentAttempt(userId, assessmentId) {
    ...
    saveAttemptAnswer(attemptId, userId, questionId, selectedOptionIndex) {
        const attempt = this.getAttemptById(attemptId);
        if (!attempt || attempt.userId !== userId)
            throw new Error('Invalid attempt');
        if (attempt.status !== 'IN_PROGRESS')
            throw new Error('Attempt is already concluded');
    ...
    recordIntegrityEvent(attemptId, userId, event) {
        const attempt = this.getAttemptById(attemptId);
        if (!attempt || attempt.userId !== userId)
            throw new Error('Invalid attempt');
        if (attempt.status !== 'IN_PROGRESS')
    ...
    submitAttempt(attemptId, userId, forcedStatus) {
        const attempt = this.getAttemptById(attemptId);
        if (!attempt || attempt.userId !== userId)
            throw new Error('Invalid attempt');
        if (attempt.status !== 'IN_PROGRESS' && !forcedStatus)
            return attempt;
        const assessment = this.getAssessmentById(attempt.assessmentId);
        if (!assessment)
            throw new Error('Assessment not found');
        ...
        attempt.timeSpentSeconds = Math.min(Math.round((now.getTime() - start) / 1000), assessment.durationMinutes * 60);
        // Evaluation
        let score = 0;
        let correctCount = 0;
    ...
    getCodingProblems(filter) {
        return this.codingProblems.filter(p => {
            ...
            if (reqStatus !== 'all' && p.status !== reqStatus)
                return false;
            if (filter?.topic && filter.topic !== 'all' && p.topic !== filter.topic)
                return false;
            if (filter?.difficulty && filter.difficulty !== 'all' && p.difficulty !== filter.difficulty)
                return false;
            if (filter?.search) {
                const q = filter.search.toLowerCase();
    ...
    getCodingProblemById(idOrSlug) {
        return this.codingProblems.find(p => p.id === idOrSlug || p.slug === idOrSlug);
    }
    createCodingProblem(problem) {
        const newProblem = {
            ...problem,
            id: `prob-${(0, uuid_1.v4)().slice(0, 8)}`,
            acceptanceRate: 75.0,
    ...
    updateCodingProblem(id, updates) {
    ...
    getCompanies(filter) {
        return this.companies.filter(c => {
            if (filter?.industry && c.industry !== filter.industry)
                return false;
            if (filter?.search) {
                const q = filter.search.toLowerCase();
    ...
    getCompanyById(id) { return this.companies.find(c => c.id === id); }
    createCompany(company) {
        const newCompany = { ...company, id: `cmp-${(0, uuid_1.v4)().slice(0, 8)}` };
    ...
    getPlacementReadiness(userId) {
        ...
        const overallReadiness = Math.round(avgAptitude * 0.15 +

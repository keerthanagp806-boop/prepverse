...
router.get('/problems/:idOrSlug', (req, res) => {
  const problem = store.getCodingProblemById(req.params.idOrSlug);
  if (!problem) {
...
router.get('/problems/:id/admin', authenticate, authorize(['ADMIN', 'INSTRUCTOR']), (req: AuthRequest, res: Response): void => {
  const problem = store.getCodingProblemById(req.params.id);
  if (!problem) {
    res.status(404).json({ error: 'Coding problem not found' });
    return;
  }
...
router.get('/submissions/me', authenticate, (req: AuthRequest, res: Response): void => {
  const { problemId } = req.query;
  const submissions = store.getUserSubmissions(req.user!.id, problemId as string);
  const enriched = submissions.map(s => {
    const p = store.getCodingProblemById(s.problemId);
    return {
      ...s,
...
router.post('/problems', authenticate, authorize(['INSTRUCTOR', 'ADMIN']), (req: AuthRequest, res: Response): void => {
  const {
    title,
    description,
    inputFormat,
  ...
  const problem = store.createCodingProblem({
    title,
    slug,
    description,
...
router.put('/problems/:id', authenticate, authorize(['INSTRUCTOR', 'ADMIN']), (req: AuthRequest, res: Response): void => {
  const existing = store.getCodingProblemById(req.params.id);
  if (!existing) {
    res.status(404).json({ error: 'Coding problem not found.' });
    return;
  }
  ...
  const updated = store.updateCodingProblem(existing.id, updates);
...
router.delete('/problems/:id', authenticate, authorize(['ADMIN']), (req: AuthRequest, res: Response): void => {
  const existing = store.getCodingProblemById(req.params.id);
  if (!existing) {
    res.status(404).json({ error: 'Coding problem not found.' });
    return;
  ...
  store.deleteCodingProblem(existing.id);
...
router.post('/problems/:id/publish', authenticate, authorize(['INSTRUCTOR', 'ADMIN']), (req: AuthRequest, res: Response): void => {
  const existing = store.getCodingProblemById(req.params.id);
  if (!existing) {
    res.status(404).json({ error: 'Coding problem not found.' });
    return;
  ...
  const updated = store.updateCodingProblem(existing.id, { status: 'published' });
...
router.post('/problems/:id/unpublish', authenticate, authorize(['INSTRUCTOR', 'ADMIN']), (req: AuthRequest, res: Response): void => {
  const existing = store.getCodingProblemById(req.params.id);
  if (!existing) {
    res.status(404).json({ error: 'Coding problem not found.' });
    return;
  ...
  const updated = store.updateCodingProblem(existing.id, { status: 'draft' });
